...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/28 21:36:33 by kaisuzuk          #+#    #+#             */
/*   Updated: 2025/07/28 21:51:54 by kaisuzuk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_command	*first_command(char *prog_name, char *cmd,
		char *filename_eof)
{
	t_command	*new_cmd;

	new_cmd = cmdnew(prog_name, cmd, NULL);
	if (!new_cmd)
		return (NULL);
	new_cmd->head = new_cmd;
...
t_command	*parse(int argc, char **argv)
{
	...
	i = 2;
	cmd_head = first_command(argv[0], argv[i], argv[i - 1]);
	if (!cmd_head)
		return (NULL);
	i++;
	while (i <= end)
	{
		if (i != end)
			cmd = middle_command(argv[0], argv[i], cmd_head);
		else
			cmd = last_command(argv[0], argv[i], argv[i + 1], cmd_head);
		if (!cmd)
